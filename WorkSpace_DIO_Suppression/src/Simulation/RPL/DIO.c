...
void Fn_NetSim_RPL_MaliciousNodeReplay(NetSim_EVENTDETAILS* pstruEventDetails) {
	...
	PRPL_CTRL_MSG ctrl_dioPdu = GET_PRPL_CTRL_MSG(pstruEventDetails->pPacket);
	PRPL_DODAG_CONFIG_OPTION dodag_config_suboption = get_option_from_msg(ctrl_dioPdu, RPLOPTION_DODAGConfiguration);
	
	int p = 0;


	while (p != dodag_config_suboption->DIORedun) {

		NetSim_PACKET* dio_pdu = fn_NetSim_Packet_CopyPacket(pstruEventDetails->pPacket);

		dio_pdu->pstruNetworkData->szDestIP = GET_BROADCAST_IP(6);
		dio_pdu->pstruNetworkData->nTTL = MAX_TTL;
		dio_pdu->dEventTime = t;
		dio_pdu->pstruNetworkData->szNextHopIp = NULL;
		dio_pdu->nTransmitterId = d;
		
		pstruEventDetails->dEventTime += 5 * MILLISECOND;
...
{
	if (pstruEventDetails->nDeviceId == MALICIOUS_NODE)
	{
		return 1;
	}
	return 0;
